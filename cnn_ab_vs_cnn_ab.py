...
NORMAL_MODEL: keras.Model = load_model('normal_model.keras', compile=False)
...
class Game:
    ...
    def __init__(self):
        self.random_cnn_last_move = None
        self.normal_cnn_last_move = None
        self.is_draw = False
    ...
    def event_loop(self):
        ...
        move = self.ai_move(self.hop, use_random_cnn=self.turn == BROWN)  #brown uses the random data. beige uses the normal data

        if move is None:
            self.end_turn()
            return

        if self.turn == BROWN and move is not None:
            print(f"Random hybrid solution moved from {move[0]} to {move[1]}")

        if self.turn == BEIGE and move is not None:
            print(f"Normal hybrid solution moved from {move[0]} to {move[1]}")
    ...
    def check_for_endgame(self):
        ...
        if brown_pieces == 0 or (beige_pieces > 0 and not brown_has_moves):
            ...
            # self.save_game_history('x_data.npy', 'y_data.npy')  # Save game history
        ...
        for epoch in range(epochs):
            ...
            while game_running:
                ...
                if self.check_for_endgame():
                    ...
                    # self.save_game_history('x_data.npy', 'y_data.npy')
    ...
    def ai_move(self, hop: bool, use_random_cnn: bool) -> Optional[tuple[int, int]]:
        best_move = None
        max_captures = 0
        possible_moves = []
        ...
        if capture_moves:
            ...
            best_score, best_move = self.board.alpha_beta(3, True, self.board, use_random_cnn)  # depth is set here

        if self.random_cnn_last_move is None and use_random_cnn:
            # if use_random_cnn:  # first random dataset trained cnn here checked
            self.random_cnn_last_move = best_move
            return best_move

        # if self.normal_cnn_last_move is None:   # second trained cnn here checked
        if self.normal_cnn_last_move is None and not use_random_cnn:
            self.normal_cnn_last_move = best_move
            return best_move
...
class Board:
    ...
    def evaluate_move(self, board_states, use_random_cnn: bool):
        ...
        return NORMAL_MODEL.predict(board_states)  #instead of evaluate board for alpha-beta this is used for second cnn
    ...
    @staticmethod
    def alpha_beta(depth: int, is_maximizing: bool, current_board: "Board", use_random_cnn: bool):
        color = BROWN if use_random_cnn else BEIGE
            ...
            for y in range(8):
                if current_board.matrix[x][y].occupant is not None and current_board.matrix[x][
                    ...
                    for move in moves:
                        ...
                        if depth > 1:
                            future_score = current_board.alpha_beta(depth - 1, not is_maximizing, new_board, use_random_cnn)[0]
                            scores.append(future_score)

        evaluation_results = current_board.evaluate_move(np.array(all_board_states), use_random_cnn)
        if scores:
            evaluation_results = np.add(evaluation_results, np.array(scores).reshape((-1, 1)))

        # Decrease the precision for better evaluation comparison
        rounded_results = np.round(evaluation_results, 3)
        # Shuffle the results to select a random max element
        np.random.shuffle(rounded_results)

        best_score_index = np.argmax(rounded_results) if is_maximizing else np.argmin(rounded_results)
        best_score = float(rounded_results[best_score_index][0])
